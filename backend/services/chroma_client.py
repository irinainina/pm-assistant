...
#                     'language': language,
#                     'document_hash': document_hash,
#                     'total_chunks': len(chunks),  # Добавляем информацию об общем количестве чанков
#                     'is_complete_document': len(chunks) == 1  # Флаг: это полный документ или часть
#                 })
#                 ids.append(chunk_id)        
...
#     def _chunk_document(self, text, chunk_size=500):
#         """
#         Создает чанки из текста.
#         Если документ меньше chunk_size слов - возвращает как один чанк.
#         Если больше - делит на части без перекрытия.
#         """
...
#         # Если документ целиком помещается в один чанк
#         if len(words) <= chunk_size:
#             return [text]
        
#         # Если документ слишком большой, делим на части
#         chunks = []
...
#                     'is_complete': metadata.get('is_complete_document', False)
#                 }
#             else:
#                 current_score = 1.0 - distance
#                 if current_score > unique_sources[source_id]['score']:
